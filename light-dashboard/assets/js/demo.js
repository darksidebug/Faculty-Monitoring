...
demo = {
    ...
    initFullCalendar: function() {
        ...
        $calendar.fullCalendar({
            ...
            selectHelper: true,
            events      : local + 'FullCalendar/load',
            ...
            eventClick : function(calEvent, jsEvent, view)
            {
                ...
                $('#txtStart').val(DateTime[0]);
                $('#start_txtTime').val(DateTime[1]);

                var DateTime2 = calEvent.end.format().split("T");
                $('#datetimepicker').val(DateTime2[0]);
                $('#end_txtTime').val(DateTime2[1]); 
            ...
            eventResize  :function(calEvent)
            {
                ...
                RecollectionOfDataGUI();
                Information('update', JustEvent, true);
            },
            editable : true,
            ...
            {
                tooltip = '<div class="tooltiptopicevent" style="width: auto; border-radius: 3px; color: #888;' +
                'height: auto; background: #fff; position: absolute; box-shadow: 1px 2px 6px rgba(0,0,0,0.2);' +
                'z-index: 10001; padding: 10px 10px 10px 10px; line-height: 200%; border: 1px solid #e3e3e3;">' + 
                '<span style="">Title&nbsp;::</span>' + ' <span style="font-weight: bolder; text-transform: uppercase; color: #666; font-size: 17px;">' 
                + data.title + '</span></br>' + '<span style=""> Start::</span>' + ' <span style="color: #777;">'
                + data.start.format('ddd, MMM. DD, YYYY h:mm A') + ' </span></br>' + '<span style="">&nbsp;End&nbsp;::</span>' + 
                '<span style="color: #777;"> ' + data.end.format('ddd, MMM. DD, YYYY h:mm A') + '</span></div>';
        ...
        function checkIfEmpty(query){
            if($('#txtTitle').val() != "" && $('.txtStart').val() != ""
                && $('.start_txtTime').val() != "" && $('.txtEnd').val() != "" 
                && $('.end_txtTime').val() != ""){
                
                RecollectionOfDataGUI();
                Information(query, JustEvent); 
            }
            else{
                swal(
                    {
                        title: "Warning",
                        text : 'No allowed empty fields.',
                        type : "warning",
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            $("#myModalEvents").modal('toggle');
                        }
                    }
                );
            } 
        }
        function RecollectionOfDataGUI(){
            JustEvent = {
                id    : $('#txtID').val(),
                title : $('#txtTitle').val(),
                start : $('#txtStart').val() + " " + $('#start_txtTime').val(),
                end   : $('#datetimepicker').val() + " " + $('#end_txtTime').val(),
            };
        }
        function Information(action, objEvent, modal){
            $.ajax({
                type:'POST',
                url: local + 'FullCalendar/cal_event/' + action,
                data: objEvent,
                success:function(data){
                    if(data){
                        swal(
                            {
                                title: "Successfull",
                                text : 'Event '+ action +' successfully.',
                                type : "success",
                                showCancelButton   : false,
                                confirmButtonColor : "#DD6B55",
                                confirmButtonText  : "Continue",
                ...
                error:function(){
                    swal(
                        {
                            title: "Failed",
                            text : 'Event failed to be changed and updated!',
                            type : "error",
                            showCancelButton   : false,
                            confirmButtonColor : "#DD6B55",
